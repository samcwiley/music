...
% Print the short hash and the local date and time of the latest commit
gitLocalDateTime =  \markup { \gitCommand "show -s --date=format:'%d%b%y %H:%M:%S' --format=%cd HEAD"}

gitLogOne = \markup { \gitCommand "log -1 --date=format:'%d%b%y %H:%M:%S' --pretty='%h %cd' " }
